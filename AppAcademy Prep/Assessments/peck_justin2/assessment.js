...
//Feedback: You can iterate through objects with a for in loop like so:
//
...
//          if (sentence[i] in object){
//              sentence[i] = object[sentence[i]]
//          }
...
function elementCount(array){
  ...
  for(var i = 0; i < array.length; i++){
    ...
    if(obj[element] === undefined){
      obj[element] = 1;
    } else { obj[element]++;//Feedback: Indentation mistake?
      }
  }
...
//Feedback:      v-- needs spaces around it
...
function magicNumbers(max){
  ...
  for(var i = 0; i < max; i++){
    ...
    selection.push(i);//Feedback: Indent this
  }
